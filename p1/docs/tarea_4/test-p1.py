...
def datos_aux(string_final, userId):
	...
	for a in range(len(string_final)):
            if(a%2==0):
                ...
                string_final2.append(string_final[a][2])
                string_final2.append(string_final[a][0])
                string_final2.append(string_final[a+1][0])
                rep.append(string_final2)
	
	for b in range(len(rep)):
                       
            r = requests.get( "http://localhost:8080/api/rest/facility_access_log/"+rep[b][1], 
                 headers={"x-hasura-admin-secret":"myadminsecretkey"})
            ...
            for c in range(len(r.json()['access_log'])): 
                if(userId!=r.json()['access_log'][c]['user']['uuid']):
                    lista2=[]
                    lista2.append(str(r.json()['access_log'][c]['user']['uuid']))
                    lista2.append(str(r.json()['access_log'][c]['timestamp']))
                    lista2.append(str(r.json()['access_log'][c]['type']))
                    lista2.append(rep[b][1])
                    lista2entera.append(lista2)

            for e in range(len(lista2entera)):
                if(lista2entera[e][2]=='IN'):
                    i=0
                    for f in range(len(lista2entera)):
                        if((lista2entera[e][0]==lista2entera[f][0]) & (i==0)):
                            listanue=[]
                            listanue.append(lista2entera[e][0])
                            listanue.append(lista2entera[e][1])
                            listanue.append(lista2entera[f][1])
                            listanue.append(lista2entera[e][3])
                            i+=1
                            listanueva.append(listanue)
            
            
            for d in range(len(listanueva)):
                fecha1=fechaparacomparar(listanueva[d][1])
                formateada1=datetime.strptime(fecha1,"%Y-%m-%d %H:%M:%S")
                fecha2=fechaparacomparar(rep[b][2])
                formateada2=datetime.strptime(fecha2,"%Y-%m-%d %H:%M:%S")
                fecha3=fechaparacomparar(listanueva[d][2])
                formateada3=datetime.strptime(fecha3,"%Y-%m-%d %H:%M:%S")
                fecha4=fechaparacomparar(rep[b][3])
                formateada4=datetime.strptime(fecha4,"%Y-%m-%d %H:%M:%S")
                
                if(formateada1<formateada2):
                    if(formateada3>formateada2):
                        guardamos=[]
                        guardamos.append(listanueva[d][0])
                        guardamos.append(listanueva[d][3])
                        guardamos.append(listanueva[d][1]+" - "+listanueva[d][2])
                        salida_final.append(guardamos)
                else:
                    if(formateada1<formateada4):
                        guardamos=[]
                        guardamos.append(listanueva[d][0])
                        guardamos.append(listanueva[d][3])
                        guardamos.append(listanueva[d][1]+" - "+listanueva[d][2])
...
def obtengo_datos_iniciales(numpag):
...
def los_datos_de_la_tabla_son_correctos(app, datos,a):
...
def then_prueba2(ctx):
	...
	try:
		datos=obtengo_datos_iniciales(numpag1)
		los_datos_de_la_tabla_son_correctos(ctx[1], datos,6)
		show_passed()
	except Exception as e:
		show_not_passed(e)
...
def then_prueba9(ctx):
	show("______________________________")
	show("TEST COMPROBACIÓN BASE DE DATOS DESCONECTADA: ERRORES CONCURRENTES")
	show("______________________________")
	show("""
		GIVEN he lanzado la aplicación""")
	ctx = lanzo_aplicacion(ctx[2])
...
def then_prueba10(ctx):
	show("______________________________")
	show("TEST CONCURRENCIA: VER LOS OBJETOS")
	show("______________________________")
	show("""
		GIVEN he lanzado la aplicación""")
	ctx = lanzo_aplicacion(ctx[2])
	show("""
	THEN veo los spinners""")
	try:
		veo_objeto(ctx[1], 'animation', "Spinner1")
		veo_objeto(ctx[1], 'animation', "Spinner 3")
		show_passed()
	except Exception as e:
		show_not_passed(e)
	stop(ctx[0])
