...
#FUNCIONES DE BOTONES
    ...
    def click_acceso(self):     #Funcion a la que se llama cuando se clica al boton de acceso
        ...
        if v.acc!=0:                                                #destruimos para que no se vayan duplicando
          v.hbox2_1.destroy()
          v.hbox2_3.destroy()
          v.hbox2_2.destroy()
        ...
        v.acc+=1
        v._show_second_screen()                                                 #se muestra la segunda pantalla

    def click_rastreo(self):                                                    #funcion que se llama al clicar al boton de rastreo
        v.pag=3
        try:
            v.lista3=v.controller.datos_inicio3(v.controller, v.uuid)               #se meten los datos en la lista 3
        except Exception as err:
                v.show_error("Error para comunicarse con la base de datos")       
           ...
           v.show_error("Fecha incorrecta") 
            ...
            if((v.fechainit[2]==v.fechainit[2]) & (v.fechainit[0]>v.fechafint[0])):
                ...
                if((v.fechainit[2]==v.fechainit[2]) & (v.fechainit[0]==v.fechafint[0]) & (v.fechainit[1]>v.fechafint[1])):
                    v.show_error("Fecha incorrecta")
        try:
           v.lista3=v.controller.on_searchdate_click(v.controller,v.fechaini,v.fechafin)    #devuelve los datos de la fecha incio y fin
...
# FUNCIONES PARA LA VENTANA 1
    ...
    def tabla(self):
        ...
        label4 = Gtk.Label(label="           Username", xalign=0)
        ...
        for self.j in range (self.elempag):                                 #recorremos los elementos de la tabla
